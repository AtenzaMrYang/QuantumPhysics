...
\begin{solution}
    ...
    \begin{equation}\label{波函数归一化条件}
        \iiint_{-\infty}^{+\infty}\left|\psi\left(\vec{r}\right)\right|^2\,\mathrm{d}x\mathrm{d}y\mathrm{d}z=1
    \end{equation}
    (1) 让波函数在全空间归一化
    $$
...
\begin{solution}
    (1) 先将波函数归一化
    $$
        \int_{-\infty}^{+\infty}|\Phi(x)|^2\mathrm{d}x
    ...
    $$
        w(x,t) = \begin{dcases}
            \frac{2}{a}\sin^2\frac{\pi x}{a}\mathrm{e}^{-\frac{\mathrm{i}}{h}Et}, & 0 \leqslant x \leqslant a \\
            0,                                                                    & \text{其他}
        \end{dcases}
    $$
    (3) 对概率密度函数求导，寻找极值点
...
\begin{solution}
    ...
    $$
        ...
        =\frac{p^2}{2m}\psi_p(x)
    $$
    这说明动量本征态$\psi_p(x)$也是Hamilton量（能量）本征态，本征值为$E=\cfrac{p^2}{2m}$\\
    (b) 粒子在初始时刻$\psi(x,0) = \psi_p(x) = \mathrm{e}^{\mathrm{i}p_0x/\hbar}$，将动量表象波函数变换到坐标表象波函数
    $$
        \psi_p(x)=\mathcal{F}[\psi(x)]
        =\frac{1}{\sqrt{2\pi\hbar}}\int_{-\infty}^{+\infty}\mathrm{e}^{\mathrm{i}px/\hbar}\left[\mathrm{e}^{-\mathrm{i}px/\hbar}\right]\,\mathrm{d}x
    ...
    $$
    (c) 对于自由粒子而言，动量本征态和能量本征态是等价的，但是题目中的$\psi(x, 0)=\delta(x)$是无穷多个动量本征态$\mathrm{e}^{\mathrm{i}px}$的叠加（即所谓叠加态），所以显然不是能量本征态。
\end{solution}
...
\begin{question}{习题12}
    ...
    \begin{enumerate}
        ...
        \item[(2)] $\psi_2(x)=\dfrac{A}{r}\mathrm{e}^{-\mathrm{i}kr}$
    \end{enumerate}
